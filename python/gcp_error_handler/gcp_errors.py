...
class GCPError(Exception):
    ...
    def __init__(self, err, prettyprint=False):
        ...
        if (isinstance(err, googleapiclient.errors.Error)):             
            #print("Interpreting googleapiclient.errors.HttpError")
            self._is_google_cloud_error = False
        elif isinstance(err, google.cloud.exceptions.GoogleCloudError):
            self._is_google_cloud_error = True
            #print("Interpreting google.cloud.exceptions.GoogleCloudError")
        else:
            raise Exception("Unsupported Exception Type: %s", err.__class__)
        ...
        if 'GOOGLE_ENABLE_ERROR_DETAIL' in os.environ or self._pretty_print:
            if (os.environ.get('GOOGLE_ENABLE_ERROR_DETAIL') == "true" or self._pretty_print) and self._is_google_cloud_error and self.grpc_status_code != None:
                ...
                for e in self._err.errors:
                    ...
                    info = self.get_google_rpc_preconditionfailure
                    if info != None:
                        resp['google.rpc.preconditionfailure-bin'] = MessageToDict(info)
                    info = self.get_google_rpc_errorinfo
                    if info != None:
                        resp['google.rpc.errorinfo-bin'] = MessageToDict(info)                                                
